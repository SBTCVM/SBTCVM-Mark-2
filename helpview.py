...
#search page generation function.
def finder():
	#get pathlist of help directory
	pathlist=sorted(os.listdir(os.path.join("VMSYSTEM", "HELP")), key=str.lower)
	#load search page template and add title
	droot = ET.fromstring(searchtemp)
	newelem=ET.Element("title")
	newelem.text="Search: " + searchstring
	droot.append(newelem)
	#iterate through help pages and search for keywords provided by user. (searchstring)
	for path in pathlist:
		...
		if path.lower().endswith(".xml"):
			...
			xroot = xtree.getroot()
			#keep track of # of matches.
			matches=0
			#search all text and title elements for keywords. (splits searchstring into list along spaces)
			for line in xroot.findall("*"):
				if line.tag=="title" or line.tag=="text":
					if any(item in line.text.lower() for item in list(searchstring.lower().split(" "))):
						matches+=1
			#if matches is over 0, add link to page with number of matches appended to title to make link text.
			if matches>0:
				#print("fun3")
