...
conftree = ET.parse(os.path.join("tutorial_data", "ENGSYSTEM.xml"))
...
try:
	mainsavtree = ET.parse(os.path.join("tutorial_data", "main.sav"))
...
def filelookup(filename):
	...
	if filename in filedict:
		...
		if (filename.lower()).endswith(".jpg") or (filename.lower()).endswith(".jpeg") or (filename.lower()).startswith("no-tr"):
			imgret=pygame.image.load(os.path.join("tutorial_data", filename)).convert()
		else:
			imgret=pygame.image.load(os.path.join("tutorial_data", filename)).convert_alpha()
...
if icontag!="NULL":
	windowicon=pygame.image.load(os.path.join("tutorial_data", icontag))
...
while quitflag==0:
	...
	if curpage!=prevpage:
		...
		tree = ET.parse(os.path.join("tutorial_data", curpage))
		root = tree.getroot()
		cachepage=prevpage
		prevpage=curpage
	...
	for fork in forktag.findall("sound"):
		...
		if masterkey in keylist:
			keylist.remove(masterkey)
			soundobj=pygame.mixer.Sound(os.path.join("tutorial_data", soundname))
			soundobj.play()
	if forksanity==1:
		forksanitycheck=1
		forksanity=0
		...
		if event.type==MOUSEBUTTONDOWN:
			...
			for f in clicklist:
				...
				if f.box.collidepoint(event.pos)==1 and event.button==1:
					if f.sfxclick==1:
						clicksound=pygame.mixer.Sound(os.path.join("tutorial_data", f.sound))
						clicksound.play()
					if f.keyid!="0":
...
mainsavtree.write(os.path.join("tutorial_data", "main.sav"))
print "Done."